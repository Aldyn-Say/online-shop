...
function addToCart($userId, $productId, $quantity = 1) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT quantity FROM users_products WHERE user_id = :user_id AND product_id = :product_id");
        ...
        if ($existingItem) {
            ...
            $stmt = $pdo->prepare("UPDATE users_products SET quantity = :quantity WHERE user_id = :user_id AND product_id = :product_id");
            $stmt->execute([
...
function removeFromCart($userId, $productId) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("DELETE FROM users_products WHERE user_id = :user_id AND product_id = :product_id");
        $stmt->execute([
            ':user_id' => $userId,
            ':product_id' => $productId
        ]);
...
function clearCart($userId) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("DELETE FROM users_products WHERE user_id = :user_id");
        $stmt->execute([':user_id' => $userId]);
...
function getCartItemsCount($userId) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("SELECT COALESCE(SUM(quantity), 0) as total_count FROM users_products WHERE user_id = :user_id");
        $stmt->execute([':user_id' => $userId]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
...
function getCartUniqueItemsCount($userId) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM users_products WHERE user_id = :user_id");
        $stmt->execute([':user_id' => $userId]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
